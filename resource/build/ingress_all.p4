...
control IngressPipeImpl (inout parsed_headers_t    hdr,
   ...
   */
    ...
    action gtpu_encap(teid_t teid,bit<32> up_ip,bit<32> gnb_ip){
        ...
        bit<256>dst_addr;
        PIT_IPV6.read(dst_addr, (bit<32>)teid);

        hdr.outer_ipv6.setValid();
        hdr.outer_ipv6.version = 6;
        hdr.outer_ipv6.traffic_class = 0;
        hdr.outer_ipv6.flow_label = 0;
        hdr.outer_ipv6.payload_len = hdr.ipv4.total_len + 24;
        hdr.outer_ipv6.next_hdr = IPV6_SRH;
        hdr.outer_ipv6.hop_limit = 64;
        hdr.outer_ipv6.dst_addr = dst_addr[127:0];
        hdr.outer_ipv6.src_addr = dst_addr[255:128];
        hdr.ethernet.ether_type = ETHERTYPE_IPV6;
        
        hdr.outer_srh.setValid();
        hdr.outer_srh.next_hdr = IP_VERSION_4;
        hdr.outer_srh.hdr_ext_len = 2;
        hdr.outer_srh.routing_type = 0x4;
        hdr.outer_srh.last_entry = 0x0;
        hdr.outer_srh.flags = 0x0;
        hdr.outer_srh.tag = 0x0;
        hdr.outer_srh.sl = 0;
        hdr.outer_srh.segment_lists = hdr.outer_ipv6.dst_addr;
    }
   ...
   { hdr.ethernet.dst_addr = dmac; }
    ...
    apply {
        ...
        if(hdr.srh.isValid()){//ip uplink
            if(srh.apply().hit)
            {
                ipv6_addr[127:0] = hdr.ipv6.src_addr;
                ipv6_addr[255:128] = hdr.ipv6.dst_addr;
                PIT_IPV6.write(local_metadata.teid, ipv6_addr);
